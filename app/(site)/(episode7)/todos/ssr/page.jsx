...
//////// REVALIDATE = false (DEFAULT)| 'force-cache' | 0 | number

export const dynamic = 'force-dynamic' // BOTH ARE SIMILAR
// export const revalidate = 0 // BOTH ARE SIMILAR
    ...
    <section className='py-20'>
      <div className='container'>
        ...
        <ul className='mt-4 flex flex-col gap-1'>
          {todos?.map(todo => (
            <li key={todo._id}>
              <Link href={`/todos/ssr/${todo.id}`}>{todo.title}</Link>
            </li>
          ))}
        </ul>
      </div>
